...
static int lynx_master_volume = 100;
static int lynx_lowpass_filter = 0;
...
static void check_variables(void)
{
   ...
   old_value             = lynx_master_volume;
   var.key               = "handy_master_volume";
   var.value             = NULL;

   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
      lynx_master_volume = atoi(var.value);

      if (initialized)
         lynx->SetVolume(lynx_master_volume);
   }

   old_value             = lynx_lowpass_filter;
   var.key               = "handy_lowpass_filter";
   var.value             = NULL;

   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
      lynx_lowpass_filter = atoi(var.value);

      if (initialized)
         lynx->SetLowpass(lynx_lowpass_filter);
   }
}
